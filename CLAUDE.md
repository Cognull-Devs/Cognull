...
- `npm run dev` — start dev server at http://localhost:8080 (see `vite.config.ts`; not the Vite default 5173, and not the 5173 mentioned in README.md)
...
**Page components are large and self-contained.** `Index.tsx` (~730 lines) and `Formulario.tsx` (~570 lines) each inline their own icon components (raw SVGs, e.g. `Menu`, `X`, `Github`, `Linkedin`), and section markup rather than splitting into many small files. Follow this existing convention rather than aggressively extracting components unless a piece is reused.

**Two different background animation approaches, do not conflate them:**
- `Index.tsx` renders a pure CSS/DOM animation: a fixed number (`ATOM_COUNT = 75`) of `.atom` divs positioned via a `--i` CSS custom property inside `.organism` containers, driven entirely by CSS (no JS animation loop). It's skipped on mobile via the `useIsMobile()` hook (`aria-hidden`, `!isMobile` gate).
- `Formulario.tsx` renders an actual `<canvas>` with a Three.js scene (`WebGLRenderer`, `Scene`, `PerspectiveCamera`) animated with GSAP tweens (`gsap.to(...)` on camera position/rotation), both loaded via dynamic `import()` inside a `useEffect`. It's gated behind `prefers-reduced-motion`, small screens (`window.innerWidth < 1024`), and low `navigator.hardwareConcurrency` (≤4) — the animation setup returns early and `three`/`gsap` are never even imported in those cases. Preserve these guards when touching that code; they exist for performance/accessibility, not by accident.
...
`README.md` describes an earlier version of this project (mentions `react-router-dom` and TanStack Query which aren't dependencies, `localhost:5173`, and omits `Formulario.tsx` from the file tree). Don't treat it as a source of truth for current architecture.
